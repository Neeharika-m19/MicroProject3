...
app.post('/update', (req,res)=>
{
    db.collection('kitchen').find().toArray((err,result) => 
    {
        ...
        {
            $set: {Quantity: (parseInt(q) + parseInt(req.body.Quantity)),ReorderLevel: parseInt(req.body.ReorderLevel),Status:req.body.Status}},
            {sort: {_id: -1}},
            (err, result) => 
            {
                if(err)
...
app.get('/AddData', (req, res) => {
    db.collection('kitchen').find().toArray((err, result) => {
        ...
        for (var i = 0; i < result.length; i++) {
            labels.push(result[i].ItemName)
            stocks.push(parseInt(result[i].Quantity))
        }
        data2.push(labels)
        data2.push(stocks)
        console.log(data2[0])
